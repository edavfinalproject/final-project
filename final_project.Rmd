...
scatter_data$energy_tot <- as.numeric(tidy_pop_energy$VALUE[tidy_pop_energy$INDICATOR_CODE == icodes[3]])
...
output$ds_scatter <- renderPlot({
    ggplot(data = tidy_subset(), aes(x=pop_growth, y=energy_tot, size=energy_tot, col=COUNTRY_NAME)) +
        geom_point() + labs(title="Population Growth vs. Energy Use", x="Population growth (%)", y="Energy use") + 
