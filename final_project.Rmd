...
output$countryhist <- renderPlot({
    ggplot(data=tab1_total_reactive(), aes(x=tab1_total_reactive()$YEAR, y=tab1_total_reactive()$VALUE)) + geom_line() + geom_point() + labs(title="Total Population over Time", x="Year", y="Population") + scale_y_continuous(labels = scales::comma) + facet_grid(INDICATOR_NAME~., scales="free")
})
```
...
usa_energy_plot <- ggplot(data=usa_energy, aes(x=usa_energy$YEAR, y=usa_energy$VALUE)) +
  geom_point() + geom_line() + xlim(1950, 2015)
usa_pop_plot <- ggplot(data=usa_pop, aes(x=usa_pop$YEAR, y=usa_pop$VALUE)) + geom_point() + geom_line() + xlim(1950, 2015)

gridExtra::grid.arrange(usa_pop_plot, usa_energy_plot)
