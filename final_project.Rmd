...
#TODOs
# Tool tip for country Name. Or just lable.
# Better axis tick lables
# Put time slider at bottom. Can you have it update quicker or there be a play button? Make figure bigger.
# Legend for region name
# Can you narrow in to look at small countries? Have dropdown option or just show box graphs?? First just manually do to see what it looks like and if its interesting.
...
scatter_data$pop_tot <- as.numeric(tidy_pop_energy$VALUE[tidy_pop_energy$INDICATOR_CODE == icodes[2]])
scatter_data$energy_percap <- as.numeric(tidy_pop_energy$VALUE[tidy_pop_energy$INDICATOR_CODE == icodes[3]])
scatter_data$energy_tot <- scatter_data$energy_percap * scatter_data$pop_tot
...
output$ds_scatter <- renderPlot({
    ggplot(data = tidy_subset(), aes(x=pop_tot, y=energy_tot, size=pop_growth, col=REGION)) +
        geom_point() + labs(title="Population vs. Energy Use", x="Population", y="Energy use") + 
        theme(legend.position="none") + coord_cartesian(xlim=c(0, max(scatter_data$pop_tot)), 
                                                          ylim=c(0, max(scatter_data$energy_tot)))
}) 
