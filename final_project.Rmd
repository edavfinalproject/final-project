...
As we started to determine what graphs we should show in this analysis, we quickly realized there were so many insightful ways we could split the data, meaning that the report would either be too long or end up skipping many key insights. Therefore, we chose to make an interactive R Shiny apps for the main part of our analysis. The R Shiny app that we created accomplishes two objectives: not only did it allowed our team to easily explore the data, but it also allows users to explore the data in ways that we might not have thought of, allowing them to draw their own conclusions.

We broke the data into separate tabs and views to explore these distinct aspects of the data. We often found ourselves jumping back and forth between views, and encourage users to do the same, as each story of the data does not need to go in chronological order. The three apps are listed below.
...
The idea to include a year slider was inspired by [a TED talk by the late Hans Rosling](https://www.gapminder.org/videos/ted-us-state-department/). In order to make the data widely accessible, we have published this R Shiny app on the Internet at the following location: https://ds3300.shinyapps.io/world_bank/.
...
However, we see there is a lot of information hidden when averaged over the entire world. Figure 5-4 below shows the relationship between energy use and population for each country in 2010. 
