...
tab1_pop_growth <- reactive({tidy_pop_energy %>% 
    filter(tab1_pop_growth$INDICATOR_NAME == "Population growth (annual %)") %>%
             filter(tab1_pop_growth$COUNTRY_NAME == tab1_countries())})

# attempt to render
output$countryhist <- renderPlot({
    ggplot(data=tab1_pop_growth(), aes(x=tab1_pop_growth$YEAR, y=tab1_pop_growth$VALUE)) +
        geom_line() +
        geom_point()
})
```
