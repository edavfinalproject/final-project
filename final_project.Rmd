...
tidy data is a product in itself. Talk about file size and how it organizes code and fits good in memory and simply provides the necessary work to analyze this. A good idea would be to make this into an r package. 
...
As shown in Figure 1 Energy use is growing. Many ay that we just need to replace our energy sources with clean sources. The toal clean energy use is ~10% today (NEED SOURCE). Therefore, to do all renewables we would need to increase by 10-fold. However, the popualtion has growth XXX times over the last 50 years. Hence, we assumed the energy growth continues (assumption only used to illustrate point) we would need to increase clean energy to 500 times what it is today. A much harder task. Hence, an extremly important componnet is stopping our energy growth.

Figure 1 (Energy use over time)
...
Figure 2 (population growth over time)

Figure 3 (Energy use per capita over time)
...
# group.colors <- c(`East Asia & Pacific` = "#e41a1c", 
#                   `Latin America & Caribbean` = "#377eb8",
#                   `North America` = "#4daf4a", 
#                   `Sub-Saharan Africa` = "#984ea3", 
#                   `Europe & Central Asia` = "#ff7f00",
#                   `Middle East & North Africa` = "ffff33",
#                   `South Asia` = "f781bf",
#                   `Energy use (kg of oil equivalent per capita)` = "e41a1c",
#                   `Population growth (annual %)` = "#377eb8",
#                   `Population, total` = "#4daf4a")
...
tidy_subset_barplot <- reactive({tidy_pop_energy %>%
          ...
          filter(value_percent_rank >= rank_range()[1] &
                 value_percent_rank <= rank_range()[2])
...
output$countryhist <- renderPlot({
    ggplot(data=tab1_total_reactive(), aes(x=tab1_total_reactive()$YEAR, y=tab1_total_reactive()$VALUE)) + 
    geom_line() + 
    geom_point() + 
    labs(title="Total Population over Time", x="Year", y="Population") +
...
ggplot(data=usa_data, aes(x=usa_data$YEAR, y=usa_data$VALUE)) + geom_point() + facet_wrap(~usa_data$INDICATOR_CODE)
