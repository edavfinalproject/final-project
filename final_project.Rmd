...
While the debate about whether human intervention has impacted the pace of climate change is all but settled (it has), the search for solutions to averting the most harmful effects of human activity on climate change is just beginning. Research has shown that one of the primary causes of climate change is the burning of fossil fuels, whose rate has been steadily growing, as shown in Figure 1. In *Prosperity Without Growth*, Tim Jackson argues that humankind cannot continue to realize advances in prosperity through the continued consumption of natural resources, pointing out that staying on the current trajectory of resource consumption is not an unsustainable endeavor [@prosperity-growth]. To the point, Jackson urges researchers and policymakers to explore sustainable avenues of global development.

A common trope which is brought up in most discussions of sustainable development is the questions of population: many observers claim that population growth is a critical contributing factor to increased energy consumption, arguing that finding ways to curb population growth could curtail the consumption of natural resources and move us closer towards sustainable development. We are skeptical of this claim and think that it needs testing. Is it fair to place the blame for increased energy consumption on developing countries with high population growth, when many Western countries, especially the United States, are consuming far more energy per person? Are population and energy consumption even correlated? If so, does this correlation hold true at a global, regional, and country level? We believe exploratory data analysis is a fantastic technique to help answer these, and other questions regarding sustainable development and population growth. Therefore, **the primary goal of this project is to explore and define the characteristics of the relationship between energy use and population over time.** 
...
All team members contriubuted at various points to data collection, data cleaning, graph creation, and theoretical discussion. We divided responsibility for the interactive component:
...
```{r Alternate way to download World Bank data via the WDI package}
...
**Missing Data: Energy use (kg of oil equivalent per capita) - Lower Middle Income**
...
**Missing Data: Energy use (kg of oil equivalent per capita) - Upper Middle Income**
...
**Missing Data: Energy use (kg of oil equivalent per capita) - High Income**
...
**Missing Data: Population growth**
```{r population growth}
...
ggplot(pop_energy_year, aes(POP_GROWTH_RATE, ENERGY_USE)) + 
    geom_point(alpha = .1) + geom_density_2d() + 
     ...
     ggtitle("Population Growth Rate vs. Energy Use", subtitle = "One observation = values for each country-year combination")
...
#### Overview
According to the [International Energy Agency](https://www.iea.org/about/faqs/renewableenergy/), 13.2% of energy in the world comes from renewable sources. Therefore, in order for clean, renewable energy to constitute 100% of total energy consumption, we would need to increase our current clean-energy capacity by 7.5 times. This figure, however, does not take into account the fact that global energy consumption is consistently growing. As shown in Figure 1-4, global energy consumption has increased approximately 2.5 times from 1975 to 2010, with an annual growth rate of 2.6%. Assuming this constant growth rate, globla energy consumption in 2100 would be 8 times the current value and 64 times larger than the current supply of clean, renewable energy sources. 
...
These above-mentioned figures suggest that slowing global energy consumption would do much to help society reach the 100% clean energy mark. As discussed in the introduction, one suggested approach to slowing the growth of energy consumption is slowing the growth of global population. Figure 4-2 shows that the global population has increased linearly from 3.7 billion in 1975 to 6.7 billion in 2010; over this same period, however, global per capita energy consumption (Figure 4-3) has only show moderate growth, from **XXX** to **XXX**. The slow growth of per-capita energy consumption suggests that increased capacity of renewable energy sources is keeping up with demand for more power, suggesting that a lower annual growth rate in global population could help society acheive 100% renewable energy.
...
ggplot(energy_capita_agg, aes(YEAR, weighted)) + geom_line() +
  ...
  coord_cartesian(xlim=c(1975, 2010), ylim=c(0,2.5))
```
...
ggplot(data=tidy_overview, aes(x=YEAR, y=VALUE, col=COUNTRY_NAME, ymin=0)) +
  ...
  labs(title="Indicators by Region", x="Year")
```
...
We immediately notice from Figure 4-# there is large inequality of energy use. China and USA are the two outliers accounting for nearly 40% of the energy use from those countries alone. The top 10 countries account for 65% and the top 50 account for 93%. This indicates our energy use is high concentrated.
...
ggplot(data=indonesia_uk_data, aes(x=YEAR, y=energy_percap, col=COUNTRY_NAME)) +
  geom_line() +
  labs(title="Figure 4-#. Energy Use Per-Capita (Indonesia vs. United Kingdom)",
       x="Year",
