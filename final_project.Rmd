...
energy_total_df <- energy_df %>% mutate(VALUE = VALUE * pop_df$VALUE) %>% mutate(INDICATOR_NAME = c("Energy use total (kg of oil equivalent)"))
...
ggplot(pop_energy_year, aes(POP_GROWTH_RATE, ENERGY_USE)) + 
     ggtitle("Total Population vs. Energy Use", 
             subtitle = "Country-Years") +
    geom_point(alpha = .1) + geom_density_2d() +
     scale_x_continuous(name="Total Population", labels = scales::comma) +
     scale_y_continuous(name="Energy use (kg of oil equivalent per capita)", 
                        labels = scales::comma)
...
ggplot(pop_energy_year %>% filter(ENERGY_USE <= 10000 & POP_GROWTH_RATE <= 2e+07),
    ...
    geom_point(alpha = .1) + geom_density_2d() + 
     ggtitle("Total Population vs. Energy Use", 
             subtitle = "Country-Years (Outliers removed)") +
     scale_x_continuous(name="Total Population", labels = scales::comma) +
     scale_y_continuous(name="Energy use (kg of oil equivalent per capita)", 
                        labels = scales::comma)
...
>>>>>>> c09e8fcf461c3a7ff0b38afbb2e5fa530baf14d2
...
ggplot(data = scatter_data[scatter_data$YEAR == 2010, ], 
  ...
  labs(title="Figure 4-#. Energy vs. Population, Zoomed (2010)",
       x="Population (millions)",
       y="Energy use (teragrams of oil equivelent)") + 
  coord_cartesian(xlim=c(0, 75), ylim=c(0, 1000)) +
  scale_x_continuous(labels = scales::comma) + scale_y_continuous(labels = scales::comma) +
  geom_text(aes(label=COUNTRY_NAME), hjust=-0.2, vjust=0)
...
In the executive summary we present a story of what the data told us. Our aim is that this story provides reference and guidence about the the relationship ebtwen energy and popualtion so that ourselves and other researches can discover  sustainable paths towards clean energy.

We clearly see that energy and population are growing and energy per capita is not. So there is a correlation.

Yet when we look at the scatter there is more to the story.

We look at distribution and find that we are only talking about a small portion

There are multiple paths.

We find some countries have found sustainable paths.
