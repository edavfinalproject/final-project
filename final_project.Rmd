...
For the sake of readability, we chose to show no code in our final report. However, all text and code can be seen in the .rmd file on github at
...
Over this same period, however, global per capita energy consumption (Figure 4-3) has growth over three times. Since the energy use per capita has growth faster than population, this suggests that population is not the main factor contributing to energy growth.
...
Yet when we look at figures such as 3-7 and 3-8 we learn that when we split by country the smooth correlation immediately goes away. 
...
The idea to include a year slider was inspired by [a TED talk by the late Hans Rosling](https://www.gapminder.org/videos/ted-us-state-department/).
